...
# Lấy Token từ GitHub Secrets hoặc Biến Môi trường
...
if not TINYBIRD_TOKEN:
    ...
    print("Hãy chạy 'set TINYBIRD_TOKEN=...' (cmd) hoặc '$env:TINYBIRD_TOKEN=...' (PowerShell)")
    exit(1) # Thoát với mã lỗi

try:
    # --- PHẦN SỬA LỖI ---
    # 1. Tạo một 'requests.Session'
    session = requests.Session()
    
    # 2. Gán User-Agent cho Session đó
    headers = {
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/58.0.3029.110 Safari/537.36"
    }
    session.headers.update(headers)
    
    print("Đang kết nối đến luồng Wikipedia SSE (dùng session)...")
    
    # 3. Khởi tạo SSEClient và truyền 'session' đã cấu hình vào
    client = SSEClient(WIKI_STREAM_URL, session=session)
